...
\vspace{-0.3em}
\textbf{NF/FF limit:} For a line aperture, $z_R = D^2/(4\lambda) = 100\lambda = 5$\,cm \scriptsize(differs from piston formula $D^2/\lambda$).

\normalsize
\begin{columns}[T]
\column{0.48\textwidth}
\colfig{image-22.png}
\scriptsize 2D field: beam confined near aperture, spreading with familiar lobe structure at larger $z$.
\column{0.48\textwidth}
\colfig{image-23.png}
\scriptsize On-axis ($x=0$): amplitude oscillates in NF (path-length interference), decays as $1/\sqrt{z}$ beyond $z_R$.
\end{columns}

\smallskip
\scriptsize \textbf{At $r=z_R$:} Angular response still shows NF artifacts; sidelobes not fully stabilised.\\
\textbf{At $r=10z_R$:} Smooth sinc pattern, first SL at $-13.2$\,dB (rectangular window value).
\end{frame}
...
\begin{itemize}\setlength\itemsep{0pt}
  \item $d=\lambda/4$: Wide main lobe ($8.5^\circ$), no grating lobes; small aperture $\Rightarrow$ poor resolution.
  \item $d=\lambda/2$: BW $\approx4.2^\circ$, max SL $=-13.2$\,dB. No grating lobes - classic design criterion.
...
for i = 1:4
    ...
    Result = analyzeBP(u, W);       % -3/-6 dB BW + max SL
    fprintf('d = %s: -3dB BW = %.2f deg, max SL = %.1f dB\n', ...
        labels{i}, rad2deg(Result.Three_dB), Result.maxSL);
end
\end{lstlisting}
...
for i = 1:length(betas)
    ...
    fprintf('beta=%.1f: -3dB=%.2f deg, maxSL=%.1f dB, WNG=%.1f dB\n',...
        betas(i), rad2deg(Result.Three_dB), Result.maxSL, 10*log10(WNG));
end
\end{lstlisting}
...
\smallskip\normalsize
Case B uses \textbf{unity weights} $\Rightarrow$ full WNG.

\smallskip
$\approx 5$\,dB SL improvement over uniform with only $0.3^\circ$ BW cost.

\smallskip
Optimising element \emph{positions} reduces SL without sacrificing noise robustness.
\end{columns}
\end{frame}
...
\colfig{image-9.png}
\scriptsize Outside the visible region ($|u|>1$) the beampattern continues. Non-uniform positions break strict periodicity but structure remains. When steered, this structure can enter the visible region.
\column{0.48\textwidth}
\textbf{Steered} ($\theta_s = 0^\circ,20^\circ,40^\circ,60^\circ$):
\colfig{image-10.png}
\scriptsize Main lobe broadens $\propto 1/\cos\theta_s$ (projected aperture shrinks). SLs grow at large steering angles. Effect is fundamental to all linear arrays.
\end{columns}
\end{frame}
...
\vspace{-0.3em}
\scriptsize
\textbf{Top:} BW in degrees increases $\propto 1/\cos\theta_s$.
\textbf{Bottom:} BW in $\sin\theta$ is approximately constant - the broadening in degrees is purely geometric.
The array's resolving power in direction-cosine space is independent of steering direction. This holds for any linear array.
\end{frame}
...
\colfig{image-16.png}
\scriptsize Optimal arrays (coloured) vs.\ dense (red).
\end{columns}

\smallskip
\scriptsize
\textbf{Key:} BW unchanged (aperture preserved). Optimised placement achieves ${\approx}5$\,dB better max SL than random thinning, trading slightly wider BW for uniformly spread sidelobes.
\end{frame}
...
\begin{itemize}\setlength\itemsep{0pt}
  ...
  \item \textbf{$d=2\lambda$:} Element zeros at $u=\pm0.5,\pm1$ coincide with grating lobes $\Rightarrow$ also suppressed at broadside. But element BW is narrow ($-3$\,dB at $u\approx0.22$, i.e.\ $\approx13^\circ$), reducing usable FoV.
\end{itemize}
\end{frame}
...
\vspace{-0.3em}
\fullfig[height=0.55\textheight]{image-20.png}

\vspace{-0.3em}
\scriptsize
Steering shifts the array factor in $u$-space, but $H_e$ is \textbf{fixed} (elements don't rotate).

\begin{itemize}\setlength\itemsep{0pt}
  \item \textbf{$d=\lambda$:} Grating lobes shift away from element zeros $\Rightarrow$ reappear. At $\theta_s=60^\circ$ a GL is partially visible but still attenuated. Usable scan $\approx\pm30^\circ\text{-}40^\circ$.
...
% H_e(kx) = sinc(kx*d/(2*pi))  (MATLAB sinc includes pi factor)
...
\scriptsize Total response = array factor $\times$ element pattern. Steering shifts $W(k_x)$ but $H_e(k_x)$ stays fixed $\Rightarrow$ GLs reappear when steered away from broadside.
\end{frame}
...
\begin{enumerate}\setlength\itemsep{3pt}
  ...
  \item \textbf{Steering (7-8):} BW in degrees $\propto 1/\cos\theta_s$; constant in $\sin\theta$.
  \item \textbf{Thinning (9):} Preserves BW but raises SL floor; optimised placement ${\approx}5$\,dB better than random.
  \item \textbf{Element directivity (12-13):} Element response suppresses GLs at broadside; steering breaks the alignment.
\end{enumerate}
\end{frame}

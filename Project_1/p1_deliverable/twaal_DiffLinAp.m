...
f0 = 3e6;
lambda = c/f0;
% Aperture, assumed to lie along the x-axis centred at the origin
D = 20*lambda;
% Discretisation
dx = lambda/10;
dz = lambda/10;
...
set(gca,'YDir','normal');
colorbar; clim([-80 0]); colormap('hot');
xlabel('x [cm]'); ylabel('z [cm]');
title('2D diffraction field [dB]');
%% 2b
z_ax = 5*dz:dz:r;
...
xlim([-90 90]); grid on
title(['Response at r = 10·z_R = ', num2str(r_far*100,'%.1f'), ' cm']);
ylabel('Magnitude');
subplot(2,1,2)
